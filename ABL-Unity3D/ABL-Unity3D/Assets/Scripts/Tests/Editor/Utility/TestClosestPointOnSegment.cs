...
{
    public static class TestClosestPointOnSegment
    {
        ...
        {
            var closestPoint = test.point.ClosestPointOnSegment2(test.endA, test.endB);
            float d2 = (closestPoint - test.point).sqrMagnitude;
            Assert.That(d2, Is.EqualTo(test.expectedDistanceSquared).Within(.1).Percent,
                $"{test.ToJsCall()}\n{test.ToConstructorString()}");

            if (closestPoint == test.endA || closestPoint == test.endB)
                return; // deviating by epsilon could actually be closer, so we'll punt the epsilon tests

            var epsilon = (test.endB - test.endA) / 100;

            var closestPlusEpsilon = closestPoint + epsilon;
            float e2 = (closestPlusEpsilon - test.point).sqrMagnitude;
            Assert.That(d2, Is.LessThanOrEqualTo(e2),
                $"{test.point} is closer to +e {closestPlusEpsilon} than {closestPoint}");

            var closestMinusEpsilon = closestPoint - epsilon;
            float m2 = (closestMinusEpsilon - test.point).sqrMagnitude;
            Assert.That(d2, Is.LessThanOrEqualTo(m2),
                $"{test.point} is closer to -e {closestMinusEpsilon} than {closestPoint}");
        }
