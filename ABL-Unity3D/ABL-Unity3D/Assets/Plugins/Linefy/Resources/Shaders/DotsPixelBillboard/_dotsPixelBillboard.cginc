...
v2fdot dot4vert (appdataL v) {
	...
	if(abs( _ViewOffset)>0.0001){	
 	...
 	float w = lerp( _WidthMultiplier, _ScreenParams.y *_WidthMultiplier*0.01, _PersentOfScreenHeightMode);
 
	float4 mp0 =  UnityObjectToClipPos( uo0);
	float2 p0 = clipToPixel( mp0 );
 	float2 uv = v.uv; 
 	
	float2 offset = lerp( v.uv1, _ScreenParams.y *v.uv1*0.01, _PersentOfScreenHeightMode);
