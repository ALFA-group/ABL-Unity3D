...
v2fpolyline polyline10vertWorldspaceXY (appdata v ) {
    v2fpolyline o;
	...
	float2 inputw = _WidthMultiplier * v.uv2  /2;
	float w = inputw.x  ;
	...
	float uvwAt = inputw.x ;
 	float uvwBt = inputw.y ;
	...
	o.color.a  =  o.color.a *  ( 1 - ndist )  ;
 	...
 	if(_id ==  -1){
		//r = p1;
		o.pos = UnityObjectToClipPos( uo1 );
	...
	if(_id ==  -2){
		...
		o.pos = UnityObjectToClipPos( float3(r.xy, uo1.z ));
